...
function configuredAdminEmails() {
  return (process.env.PROGRAM_ADMIN_EMAILS ?? "")
    .split(",")
...
export async function ensureUserProfile(user: AuthenticatedUser) {
  ...
  if (!existing) {
    ...
    const row = { userId: user.userId, displayName: user.displayName, email: user.email, role, actingRole: role, createdAt: now, updatedAt: now };
